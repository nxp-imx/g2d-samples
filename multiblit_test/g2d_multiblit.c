...
int main(void) {
  ...
  *((int *)((long)d_buf->buf_vaddr)) = 0x0;
  ...
  diff = ((tv2.tv_sec - tv1.tv_sec) * 1000000 + (tv2.tv_usec - tv1.tv_usec)) /
         TEST_LOOP;
  printf(
      "g2d multiblit 4 layers time %dus, %dfps, %d(4 * %d)Mpixel/s ........\n",
      diff, 1000000 / diff, test_width * test_height / diff * 4,
      test_width * test_height / diff);
  ...
  diff = ((tv2.tv_sec - tv1.tv_sec) * 1000000 + (tv2.tv_usec - tv1.tv_usec)) /
         TEST_LOOP;
  printf(
      "g2d multiblit 8 layers time %dus, %dfps, %d(8 * %d)Mpixel/s ........\n",
      diff, 1000000 / diff, test_width * test_height / diff * 8,
      test_width * test_height / diff);
  ...
  for (n = 0; n < layers; n++) {
    for (i = 0; i < test_height; i++) {
      for (j = 0; j < test_width; j++) {
        *(int *)(((long)mul_s_buf[n]->buf_vaddr) + (i * test_width + j) * 4) =
            (i * test_width + j) + n * 10;
      }
    }
  }
  ...
  for (i = 0; i < test_height; i++) {
    for (j = 0; j < test_width; j++) {
      ...
      int correct_val = *(int *)(((long)mul_s_buf[layer]->buf_vaddr) +
                                 (i * test_width + j) * 4);
      int rotated_val =
          *(int *)(((long)d_buf->buf_vaddr) + (i * test_width + j) * 4);
  ...
  printf(
      "  0 rotation 8 layers time %dus, %dfps, %d(8 * %d)Mpixel/s ........\n",
      diff, 1000000 / diff, test_width * test_height / diff * 8,
      test_width * test_height / diff);
  ...
  diff = ((tv2.tv_sec - tv1.tv_sec) * 1000000 + (tv2.tv_usec - tv1.tv_usec)) /
         TEST_LOOP;
  printf(
      "  0 rotation 4 layers time %dus, %dfps, %d(4 * %d)Mpixel/s ........\n",
      diff, 1000000 / diff, test_width * test_height / diff * 4,
      test_width * test_height / diff);
  ...
  for (i = 0; i < test_width; i++) {
    for (j = 0; j < test_height; j++) {
      *(int *)(((long)mul_s_buf[layers - 1]->buf_vaddr) +
               (i * test_height + j) * 4) =
          i * test_height + j + (layers - 1) * 10;
    }
  }
  ...
  for (i = 0; i < test_height; i++) {
    for (j = 0; j < test_width; j++) {
      int correct_val = *(int *)(((long)mul_s_buf[layers - 1]->buf_vaddr) +
                                 (j * test_height + test_height - i - 1) * 4);
      int rotated_val =
          *(int *)(((long)d_buf->buf_vaddr) + (i * test_width + j) * 4);
      if (correct_val != rotated_val) {
  ...
  diff = ((tv2.tv_sec - tv1.tv_sec) * 1000000 + (tv2.tv_usec - tv1.tv_usec)) /
         TEST_LOOP;
  printf(
      " 90 rotation 4 layers time %dus, %dfps, %d(4 * %d)Mpixel/s ........\n",
      diff, 1000000 / diff, test_width * test_height / diff * 4,
      test_width * test_height / diff);
  ...
  for (i = 0; i < test_height; i++) {
    for (j = 0; j < test_width; j++) {
      int correct_val =
          *(int *)(((long)mul_s_buf[layers - 1]->buf_vaddr) +
                   ((test_height - i - 1) * test_width + (test_width - j - 1)) *
                       4);
      int rotated_val =
          *(int *)(((long)d_buf->buf_vaddr) + (i * test_width + j) * 4);
  ...
  diff = ((tv2.tv_sec - tv1.tv_sec) * 1000000 + (tv2.tv_usec - tv1.tv_usec)) /
         TEST_LOOP;
  printf(
      "180 rotation 4 layers time %dus, %dfps, %d(4 * %d)Mpixel/s ........\n",
      diff, 1000000 / diff, test_width * test_height / diff * 4,
      test_width * test_height / diff);
  ...
  for (i = 0; i < test_height; i++) {
    for (j = 0; j < test_width; j++) {
      int correct_val = *(int *)(((long)mul_s_buf[layers - 1]->buf_vaddr) +
                                 ((test_width - j - 1) * test_height + i) * 4);
      int rotated_val =
          *(int *)(((long)d_buf->buf_vaddr) + (i * test_width + j) * 4);
      if (correct_val != rotated_val) {
        printf(
  ...
  diff = ((tv2.tv_sec - tv1.tv_sec) * 1000000 + (tv2.tv_usec - tv1.tv_usec)) /
         TEST_LOOP;
  printf(
      "270 rotation 4 layers time %dus, %dfps, %d(4 * %d)Mpixel/s ........\n",
      diff, 1000000 / diff, test_width * test_height / diff * 4,
      test_width * test_height / diff);
  ...
  for (i = 0; i < test_height; i++) {
    for (j = 0; j < test_width; j++) {
      int correct_val = *(int *)(((long)mul_s_buf[layers - 1]->buf_vaddr) +
                                 ((test_height - i - 1) * test_width + j) * 4);
      int rotated_val =
          *(int *)(((long)d_buf->buf_vaddr) + (i * test_width + j) * 4);
      if (correct_val != rotated_val) {
  ...
  for (i = 0; i < test_height / 2; i++) {
    for (j = 0; j < test_width; j++) {
      char *sp0 =
          (char *)(((long)s_buf->buf_vaddr) + (i * test_width + j) * 4 * 2);
      char *sp1 =
          (char *)(((long)s_buf->buf_vaddr) + (i * test_width + j) * 4 * 2 + 4);
      ...
      if (abs(Y0 - p[0]) > 2 || abs(U0 - p[1]) > 2 || abs(Y1 - p[2]) > 2 ||
          abs(V0 - p[3]) > 2) {
  ...
  printf("mode 1:  src: "
         "G2D_ZERO,G2D_ZERO,G2D_ZERO,G2D_ZERO,G2D_ZERO,G2D_ZERO,G2D_ZERO,G2D_"
         "ZERO dst: G2D_ONE\n");
  printf("mode 2:  src: G2D_ONE, G2D_ZERO,G2D_ZERO,G2D_ONE, "
         "G2D_ZERO,G2D_ZERO,G2D_ZERO,G2D_ONE  dst: G2D_ONE\n");
  printf("mode 3:  src: G2D_ONE, G2D_ONE, G2D_ONE, G2D_ONE, G2D_ONE, G2D_ONE, "
         "G2D_ONE, G2D_ONE  dst: G2D_ONE\n");
  printf("mode 4:  src: "
         "G2D_ZERO,G2D_ZERO,G2D_ZERO,G2D_ZERO,G2D_ZERO,G2D_ZERO,G2D_ZERO,G2D_"
         "ZERO dst: G2D_ONE_MINUS_SRC_ALPHA\n");
  printf("mode 5:  src: G2D_ONE, G2D_ONE, G2D_ONE, G2D_ONE, G2D_ONE, G2D_ONE, "
         "G2D_ONE, G2D_ONE  dst: G2D_ONE_MINUS_SRC_ALPHA\n");

  for (n = 0; n < layers; n++) {
    for (i = 0; i < test_height; i++) {
      for (j = 0; j < test_width; j++) {
        char *p = (char *)(((long)mul_s_buf[n]->buf_vaddr) +
                           (i * test_width + j) * 4);
  ...
  for (i = 0; i < test_height; i++) {
    for (j = 0; j < test_width; j++) {
      char *sp0 =
          (char *)(((long)mul_s_buf[0]->buf_vaddr) + (i * test_width + j) * 4);
      char *sp3 =
          (char *)(((long)mul_s_buf[3]->buf_vaddr) + (i * test_width + j) * 4);
      char *sp7 =
          (char *)(((long)mul_s_buf[7]->buf_vaddr) + (i * test_width + j) * 4);
      char *p = (char *)(((long)d_buf->buf_vaddr) + (i * test_width + j) * 4);
  ...
  for (i = 0; i < test_height; i++) {
    for (j = 0; j < test_width; j++) {
      char *sp0 =
          (char *)(((long)mul_s_buf[0]->buf_vaddr) + (i * test_width + j) * 4);
      char *sp1 =
          (char *)(((long)mul_s_buf[1]->buf_vaddr) + (i * test_width + j) * 4);
      char *sp2 =
          (char *)(((long)mul_s_buf[2]->buf_vaddr) + (i * test_width + j) * 4);
      char *sp3 =
          (char *)(((long)mul_s_buf[3]->buf_vaddr) + (i * test_width + j) * 4);
      char *sp4 =
          (char *)(((long)mul_s_buf[4]->buf_vaddr) + (i * test_width + j) * 4);
      char *sp5 =
          (char *)(((long)mul_s_buf[5]->buf_vaddr) + (i * test_width + j) * 4);
      char *sp6 =
          (char *)(((long)mul_s_buf[6]->buf_vaddr) + (i * test_width + j) * 4);
      char *sp7 =
          (char *)(((long)mul_s_buf[7]->buf_vaddr) + (i * test_width + j) * 4);
      char *p = (char *)(((long)d_buf->buf_vaddr) + (i * test_width + j) * 4);
      if (abs(sp0[0] + sp1[0] + sp2[0] + sp3[0] + sp4[0] + sp5[0] + sp6[0] +
              sp7[0] + 0x64 - p[0]) > 2 ||
          abs(sp0[1] + sp1[1] + sp2[1] + sp3[1] + sp4[1] + sp5[1] + sp6[1] +
              sp7[1] + 0x64 - p[1]) > 2 ||
          abs(sp0[2] + sp1[2] + sp2[2] + sp3[2] + sp4[2] + sp5[2] + sp6[2] +
              sp7[2] + 0x64 - p[2]) > 2 ||
          abs(sp0[3] + sp1[3] + sp2[3] + sp3[3] + sp4[3] + sp5[3] + sp6[3] +
              sp7[3] + 0x64 - p[3]) > 2) {
  ...
  for (i = 0; i < test_height; i++) {
    for (j = 0; j < test_width; j++) {
      char *p = (char *)(((long)d_buf->buf_vaddr) + (i * test_width + j) * 4);
      if ((abs(p[0] - 60) > 3) || (abs(p[1] - 60) > 3) ||
          (abs(p[2] - 60) > 3) || (abs(p[3] - 60) > 3)) {
  ...
  for (i = 0; i < test_height; i++) {
    for (j = 0; j < test_width; j++) {
      char *p = (char *)(((long)d_buf->buf_vaddr) + (i * test_width + j) * 4);
      if ((abs(p[0] - 154) > 3) || (abs(p[1] - 154) > 3) ||
          (abs(p[2] - 154) > 3) || (abs(p[3] - 154) > 3)) {
  ...
  for (i = 0; i < test_height; i++) {
    for (j = 0; j < test_width; j++) {
      char *sp0 =
          (char *)(((long)mul_s_buf[0]->buf_vaddr) + (i * test_width + j) * 4);
      char *p = (char *)(((long)d_buf->buf_vaddr) + (i * test_width + j) * 4);
      if (abs(sp0[0] * 0x80 / 0xff + 0x64 - p[0]) > 3 ||
          abs(sp0[3] * 0x80 / 0xff + 0x64 - p[3]) > 3) {
  ...
  for (i = 0; i < test_height; i++) {
    for (j = 0; j < test_width; j++) {
      char *sp0 =
          (char *)(((long)mul_s_buf[0]->buf_vaddr) + (i * test_width + j) * 4);
      char correct_val =
          (sp0[0] * 0x80 / 0xff) + 0x64 - (sp0[0] * 0x80 * 0x64 / 0xff / 0xff);
      char *p = (char *)(((long)d_buf->buf_vaddr) + (i * test_width + j) * 4);
