 ...
 */
static bool parseWidthHeight(const char* widthHeight, int* pWidth,
        int* pHeight) {
    long width, height;
    char* end;
 ...
 * Returns true on success.
 */
static bool parseFormat(const char *fmtStr, int *pSrcFmt,
        int *pDstFmt) {
...
int main (int argc, char *  *argv)
{
    ...
    while (true) {
        int optionIndex;
        int ic = getopt_long(argc, argv, "hvw:d:s:f:i:1",
        ...
        switch (ic) {
            ...
            case 'h':
                ...
                break;
            case 's':
                if (!parseWidthHeight(optarg, &srcWidth, &srcHeight)) {
                ...
                if (srcWidth == 0 || srcHeight == 0) {
                    fprintf(stderr,
                            "Invalid size %ux%u, w and h may not be zero\n",
                            srcWidth, srcHeight);
                    return 2;
                }
                break;
            case 'd':
                if (!parseWidthHeight(optarg, &dstWidth, &dstHeight)) {
                    fprintf(stderr, "Invalid size '%s', must be w x h\n",
                            optarg);
                    return 2;
                }
                ...
                break;
            case 'w':
                if (!parseWidthHeight(optarg, &srcStride, &dstStride)) {
                    fprintf(stderr, "Invalid size '%s', must be w x h\n",
                            optarg);
                    return 2;
                }
            ...
            case 'f':
                if (!parseFormat(optarg, &srcFmt, &dstFmt)) {
                    fprintf(stderr, "Invalid format '%s', must be src-dst\n"
    ...
    if (0 == srcStride) srcStride = srcWidth;
    if (0 == dstWidth) dstWidth = srcWidth;
    if (0 == dstHeight) dstHeight = srcHeight;
    if (0 == dstStride) dstStride = dstWidth;
    ...
    printf("inFile=%s, src, wxh=%dx%d, stride=%d, srcFmt=%d\n",
            inFile, srcWidth, srcHeight, srcStride, srcFmt);
    printf( "outFile=%s, dst, wxh=%dx%d, stride=%d, dstFmt=%d\n",
            outFile, dstWidth, dstHeight, dstStride, dstFmt);
    ////////////////////----------------------------------------
...
int CreateG2DBuffer (g2d_buf_t *g2d_buf_ptr,  int size)
{
...
#if G2D_CACHEABLE
    //alloc physical contiguous memory for source image data with cacheable attribute
    ...
    g2d_buf_ptr->data = (unsigned char *)((unsigned long) g2d_buf_ptr->g2d_buf->buf_vaddr);
    return 0;
}
